...
var ModalDetalle = function () {
    ...
    var abrirModal = function (id) {
        $.ajax({
            cache: false,
            type: 'GET',
            url: siteLocation + "Instalaciones/Detalle?id=" + id,
            success: function (result) {
                if (result.error) {
                    ...
                    $('#modal_registro #modalLabelTitle').html("Detalle de Instalación");
...
var DescargarDocumento = function (url) {
    var generar = function (url) {
        $.ajax({
            ...
            processData: false,
            contentType: false,
            url: '/Instalaciones/DescargarDocumento?url=' + url,
            success: function (result) {
...
var Modal = function () {
    ...
    var abrirModal = function () {
        ...
        $.ajax({
            cache: false,
            type: 'GET',
            url: siteLocation + "Instalaciones/Registro",
            success: function (result) {
                if (result.error) {
                    toastr.error(result.errorDescription, "SMADSOT");
                } else {
                    ...
                    $('#modal_registro').addClass('modal-md');
                    $('#modal_registro #modalLabelTitle').html("Registrar Instalación");
                    $('#modal_registro .modal-body').html("");
                    $('#modal_registro .modal-body').html(result.result);
                    $('#modal_registro').modal('show');
    ...
    var listeners = function () {
        ...
        $("#autocomplete").select2({
            ...
            width: '100%',
            ajax: {
                url: 'Instalaciones/ProveedorAutocomplete',
                dataType: "json",
                delay: 2000,
    ...
    var guardar = function () {
        validation.validate().then(async function (status) {
            ...
            if (status === 'Valid') {
                ...
                formData.append("IdProveedor", $("#_idProveedor").text());
                var files = [];
                ...
                $.ajax({
                    cache: false,
                    type: 'POST',
                    url: siteLocation + "Instalaciones/CreateInstalacion",
                    processData: false,
                    contentType: false,
                    data: formData,
                    success: function (result) {
            ...
            form,
            {
                fields: {
                    Autocomplete: {
                        validators: {
                            notEmpty: {
                                message: 'Debe agregar un proveedor'
                            },
                        }
                    }
