...
namespace Smadot.Web.Helper.Operaciones.Catalogo
{
    ...
    {
        public static async Task<ResponseViewModel> Consulta(IProxyWebAPI _proxyWebAPI, MarcaVehiculoRequest req)
        {
            try
            {
                var query = _proxyWebAPI.ToQueryString(req);
                var request = await _proxyWebAPI.GetAsyncData<ResponseGrid<SelectListItem>>("gateway/Catalogo/MarcaVehiculo" + query);
