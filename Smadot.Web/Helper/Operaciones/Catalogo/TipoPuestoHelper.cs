...
{
    public class TipoPuestoHelper
    {
        public static async Task<ResponseViewModel> Consulta(IProxyWebAPI _proxyWebAPI, TipoPuestoRequest req)
        {
            try
            {
                var query = _proxyWebAPI.ToQueryString(req);
                var request = await _proxyWebAPI.GetAsyncData<List<TipoPuestoResponse>>("gateway/TipoPuesto" + query);
