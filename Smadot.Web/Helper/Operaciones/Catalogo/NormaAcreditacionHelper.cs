...
{
    public class NormaAcreditacionHelper
    {
        public static async Task<ResponseViewModel> Consulta(IProxyWebAPI _proxyWebAPI, NormaAcreditacionRequest req)
        {
            try
            {
                var query = _proxyWebAPI.ToQueryString(req);
                var request = await _proxyWebAPI.GetAsyncData<List<NormaAcreditacionResponse>>("gateway/NormaAcreditacion" + query);
